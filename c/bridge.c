...
const char *bridge_last_error(void) {return last_error;}

void bridge_set_qemu_path(const char *path) {if (path && *path) snprintf(qemu_path, sizeof(qemu_path), "%s", path);}

void bridge_set_app_path(const char *path) {if (path && *path) snprintf(app_path, sizeof(app_path), "%s", path);}
...
) 
    {
        ...
        if (write_all(in_pipe[1], in, in_sz) != 0) 
        {
            ...
            if (err_buf) 
            {
                ...
                free(err_buf);
            } else {set_error("write stdin failed: %s", strerror(saved_errno));}
            return -1;
        }
        close(in_pipe[1]);
